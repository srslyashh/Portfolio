...
{
  struct queue_from_stacks* holder = malloc(sizeof(struct queue_from_stacks));

  holder->s1 = stack_create();
  holder->s2 = stack_create();
...
int queue_from_stacks_isempty(struct queue_from_stacks* queue) 
{
  ...
  else if(stack_isempty(queue->s1) == 0 || stack_isempty(queue->s2) == 0)
  {
    returnVal = 0;
  }
...
void swappyData(struct stack* firstQ, struct stack* secondQ)
{
  while(stack_isempty(firstQ) == 0)
  {
    stack_push(secondQ, stack_pop(firstQ));
  }
}
...
void queue_from_stacks_enqueue(struct queue_from_stacks* queue, int value) 
{
  ...
  // while s1 isn't empty, push everything to stack 2.
  swappyData(queue->s1, queue->s2);
  // push the newest element to s1.
  stack_push(queue->s1, value);
  // push all the data back to s1.
  swappyData(queue->s2, queue->s1);
}
...
{
  if(queue == NULL || queue_from_stacks_isempty(queue) == 1)
  {
...
{
  if(queue == NULL || stack_isempty(queue->s1))
  {
    printf("Error: Queue is NULL or empty\n");
    exit(1);
  }
