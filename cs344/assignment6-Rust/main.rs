...
fn main() {
    ...
    let mut index_c = 0;
    let mut first_tholder = Vec::new();

    // this is set to two as we split the data into two.
    while index_c < 2
    {
        // cloning the thread before passing it to map_data().
        let thread_clone = xs[index_c].clone();
        let thr = thread::spawn(move || map_data(&thread_clone));
        index_c += 1;
        // push the thread into first_tholder so we can use it later.
        first_tholder.push(thr);
    }

    for t in first_tholder
    {
        // Getting the return value from map_data
        let ret_holder = t.join().unwrap();
        // pushing the values to intermediate_sums
        intermediate_sums.push(ret_holder);
    }
    ...
    let holder = partition_data(num_partitions, &v);

    // 2. Calls print_partition_info to print info on the partitions that have been created
    print_partition_info(&holder);

    // 3. Creates one thread per partition and uses each thread to concurrently process one partition
    let mut index_counter = 0;
    let mut second_sum: Vec<usize> = Vec::new();
    let mut thread_one;
    let mut thread_holder = Vec::new();

    // index counter starts from 0 until the num_partitions -1.
    while index_counter < num_partitions
    {
        // cloning the thread before we pass it to map_data()
        let cloned_ver = holder[index_counter].clone();
        thread_one = thread::spawn(move || map_data(&cloned_ver));
        index_counter+= 1;
        // pushing the thread to thread_tholder so we can use it later.
        thread_holder.push(thread_one);
    }

    // 4. Collects the intermediate sums from all the threads

    // Now, we get the return values from the thread.
    for thread in thread_holder
    {
        let return_val = thread.join().unwrap();
        // pushing the values to second_sum so we can call reduce_data on it later.
        second_sum.push(return_val);
    }

    // 5. Prints information about the intermediate sums
    println!("Intermediate sums = {:?}", second_sum);

    // 5. Calls reduce_data to process the intermediate sums
    let sum_two = reduce_data(&second_sum);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let val;
     ...
     // if there is a remainder, then val = 1. Otherwise, it is 0.
     // Val will be used later on in the function below.
    ...
    for _j in 0.. num_partitions
    {
        ...
        // If val equals 0, then there is no remainder from num_elements / num_partition
        // division. Run it normally.
        if val == 0
        {
        ...
        // Else, then there must be a remainder from the division. Make sure that the partitions
        // sum up to the total elements.
        else
        {
            for _i in 0..partition_size
            {
            ...
            // If there is remainder, add +1 element to the vector.
            if remain > 0
            {
                x1.push(v[counter]);
                counter += 1;
