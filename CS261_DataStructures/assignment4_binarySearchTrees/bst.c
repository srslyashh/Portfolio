...
int bstSizeNode(struct bst_node* currNode)
{
  if(currNode == NULL)
  {
    return 0;
  }
  // recursively count the size node.
  return 1 + bstSizeNode(currNode->right) + bstSizeNode(currNode->left);
}

int bst_size(struct bst* bst) 
{
  return bstSizeNode(bst->root);
}
...
int bstNodeHeight(struct bst_node* currNode)
{
  if(currNode == NULL)
  {
    return -1;
  ...
  // recursively find the maxheight of the node's right and left
  // side, and add it to the current node's height.
  return 1 + maxHeight(bstNodeHeight(currNode->left), bstNodeHeight(currNode->right));
}
...
int bstNodeSum(int sum, int total, struct bst_node* currNode, struct bst_node *root, int dir)
{
  if(currNode == NULL && total == sum)
  {
    // if direction is left, check if the root->right is NULL
    // to ensure it is leaf.
    if(dir == 1 && root->right == NULL)
    {
      return 1;
    }
    // if direction is right, check if the root->left is NULL
    // to ensure it is leaf.
    else if(dir == 2 && root->left == NULL)
    {
      return 1;
    }
  }
  ...
  int left = bstNodeSum(sum, total + currNode->val, currNode->left, currNode, 1);
  int right = bstNodeSum(sum, total + currNode->val, currNode->right, currNode, 2);
...
{
  int total = 0;
  return bstNodeSum(sum, total, bst->root, NULL, 0);
}
...
void in_order(struct stack* stacks,struct bst_node* nodey)
{
  //  if the right hand-side is not null, keep on traversing.
  if(nodey->right != NULL)
  {
    in_order(stacks, nodey->right);
  }
  // add the value!
  stack_push(stacks,(void *)&nodey->val);
  if(nodey->left != NULL)
  {
    in_order(stacks, nodey->left);
  }
}

struct bst_iterator* bst_iterator_create(struct bst* bst) 
{
  struct bst_iterator* bstIt = malloc(sizeof(struct bst_iterator));
  assert(bstIt);
  bstIt->s = stack_create();
  in_order(bstIt->s, bst->root);
...
{
  return (stack_isempty(iter->s) != 1) ? 1 : 0;
}
