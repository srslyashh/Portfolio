...
{
  char alphabet[27] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ ";
  // minus it by 1, because we shifted the character array 
  // by 1.
...
/*
  encryptData:
    encrypt data that is received by client.
*/
void encryptData(char *message, char *key, char *dest, int dataLength)
{
  ...
  for(int i = 0; i < dataLength; i++)
  {
    ...
    //printf("%d. m (%c): %d, k(%c): %d\n", i, message[i],messageIndex, key[i], keyIndex);

    // get the size of the encrypted message by adding the index 
    // of the key and the message. 
    holder = getIndex(message[i]) + getIndex(key[i]);

    // if holder sum is bigger than 27(it's not 26) because we shift 
    // the character array by 1. A starts at 1 instead of 0.
    if(holder > 27)
    {
      holder = holder - 27;
    }

    // Decrypting the data by getting the alphabet by sending the sum index
    // to getAlpha.
  ...
  // copy the decrypted message to destination
  strcpy(dest, decrypt);
}
            ...
            else
            {
              /*
                The authentication key sent by the client should be in this format: 
                  ...
                  so for enc_client, the authentication key would be:
                  enc_client/dataLength\n.
              */
              autKey = strtok_r(authenticate, "/", &save);
              
              // if it is enc_client, then proceed to tokenize the rest of the message 
              // that was received. 
              if(strcmp(autKey, "enc_client") == 0)
              {
                ...
                encryptData(data, key, encrypted, dataLength);
                //printf("Decrypt: %s", encrypted);

                // sending encrypted data to client. 
                sendData(dataLength, encrypted, connectionSocket);
              }
              // if it is not enc_client, don't connect and send an error message to client.
